...
public class HelperFunctions {
	...
	public static boolean writeToLog(String logFile,String text){
		
		try {
			if(Files.notExists(Paths.get("./Logs/"+ logFile + "Log.txt"))){
				Files.write(Paths.get("./Logs/"+ logFile + "Log.txt"), 
						(OffsetDateTime.now().toString() + text ).getBytes());
			}
			else{
				Files.write(Paths.get("./Logs/"+ logFile + "Log.txt"), 
					(OffsetDateTime.now().toString() + text).getBytes(), StandardOpenOption.APPEND);
			}
		}catch (IOException e) {
			return false;
